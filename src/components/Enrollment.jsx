            ...
            {[1, 2, 3].map((step) => (
              <React.Fragment key={step}>
                <div className="progress-step">
                  <div className={`step-circle ${step <= currentStep || isEnrolled ? 'active' : 'inactive'} ${step < currentStep || isEnrolled ? 'completed' : ''}`}>
                    {step < currentStep || isEnrolled ? <CheckIcon /> : step}
                  </div>
                  <div className={`step-label ${step === currentStep && !isEnrolled ? 'active' : step < currentStep || isEnrolled ? 'completed' : 'inactive'}`}>
                    {step === 1 && "Learner's\nInformation"}
                    {step === 2 && "Parent/Guardian's\nInformation"}
                    {step === 3 && "Confirmation"}
                  </div>
                </div>
                {step < 3 && <div className={`progress-line ${step < currentStep || isEnrolled ? 'active' : 'inactive'}`}></div>}
              </React.Fragment>
            ))}
