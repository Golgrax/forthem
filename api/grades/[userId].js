...
function initializeDatabase(dbInstance, isFileDb = true) {
    dbInstance.serialize(() => {
        ...
        dbInstance.run(`CREATE TABLE IF NOT EXISTS grades (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            subject TEXT,
            grade TEXT,
            user_id INTEGER,
        ...
        // Check if grades exist, if not insert default grades
        const check = 'SELECT COUNT(*) as count FROM grades';
        dbInstance.get(check, (err, row) => {
            ...
            if (row.count === 0) {
                console.log("No grades found, inserting default grades...");
                const insertGrades = 'INSERT INTO grades (subject, grade, user_id) VALUES (?, ?, ?)';
                dbInstance.run(insertGrades, ["Science", "A", 1]);
                dbInstance.run(insertGrades, ["Filipino", "A", 1]);
                dbInstance.run(insertGrades, ["GMRC", "A", 1]);
                console.log("Default grades inserted");
            } else {
                console.log("Grades already exist in database");
            }
        });
    });
}
