...
function initializeDatabase(dbInstance, isFileDb = true) {
    dbInstance.serialize(() => {
        dbInstance.run(`CREATE TABLE IF NOT EXISTS modules (
        ...
        // Check if modules exist, if not insert default modules
        const check = 'SELECT COUNT(*) as count FROM modules';
        dbInstance.get(check, (err, row) => {
            ...
            if (row.count === 0) {
                console.log("No modules found, inserting default modules...");
                const insertModules = 'INSERT INTO modules (subject, teacher_name) VALUES (?, ?)';
                dbInstance.run(insertModules, ["Science", "Ms. Cha"]);
                dbInstance.run(insertModules, ["Filipino", "Ms. Cha"]);
                dbInstance.run(insertModules, ["GMRC", "Ms. Cha"]);
                dbInstance.run(insertModules, ["Mathematics", "Ms. Cha"]);
                dbInstance.run(insertModules, ["Araling Panlipunan", "Ms. Cha"]);
                dbInstance.run(insertModules, ["MAPEH", "Ms. Cha"]);
                dbInstance.run(insertModules, ["EPP", "Ms. Cha"]);
